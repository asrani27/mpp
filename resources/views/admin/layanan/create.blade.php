...
@section('title', 'Tambah Layanan - Ependri')
...
<div class="bg-white rounded-2xl shadow-xl overflow-hidden">
    ...
    <div class="px-6 py-5 bg-gradient-to-r from-emerald-500 to-emerald-600 flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-white">Tambah Layanan Baru</h1>
            <p class="text-emerald-100 text-sm mt-1">Tambah data layanan permohonan baru</p>
        </div>
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('admin.layanan.store') }}" class="p-6 space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
                <label for="nama" class="block text-sm font-semibold text-gray-700 mb-2">
                    <i class="fas fa-headset mr-2 text-emerald-500"></i>Nama Layanan *
                </label>
                <input type="text" id="nama" name="nama" value="{{ old('nama') }}" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all" placeholder="Masukkan nama layanan">
            </div>

            <!-- Lama Proses -->
            <div>
                <label for="lama_proses" class="block text-sm font-semibold text-gray-700 mb-2">
                    <i class="fas fa-clock mr-2 text-emerald-500"></i>Lama Proses *
                </label>
                <input type="text" id="lama_proses" name="lama_proses" value="{{ old('lama_proses') }}" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all" placeholder="Contoh: 3x24 Jam">
            </div>
        ...
        <div>
            ...
            <textarea id="deskripsi" name="deskripsi" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all resize-none" placeholder="Masukkan deskripsi layanan (opsional)">{{ old('deskripsi') }}</textarea>
        </div>

        <!-- Syarat -->
        <div>
            <label for="syarat" class="block text-sm font-semibold text-gray-700 mb-2">
                <i class="fas fa-list-check mr-2 text-emerald-500"></i>Syarat
            </label>
            <textarea id="syarat" name="syarat" rows="4" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all resize-none" placeholder="Masukkan syarat layanan (opsional)">{{ old('syarat') }}</textarea>
