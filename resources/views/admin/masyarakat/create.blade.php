...
@section('title', 'Tambah Masyarakat - Ependri')
...
<div class="bg-white rounded-2xl shadow-xl overflow-hidden">
    ...
    <div class="px-6 py-5 bg-gradient-to-r from-emerald-500 to-emerald-600 flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-white">Tambah Masyarakat Baru</h1>
            <p class="text-emerald-100 text-sm mt-1">Tambah data masyarakat baru</p>
        </div>
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('admin.masyarakat.store') }}" class="p-6 space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
                <label for="nik" class="block text-sm font-semibold text-gray-700 mb-2">
                    <i class="fas fa-id-card mr-2 text-emerald-500"></i>NIK *
                </label>
                <input type="text" id="nik" name="nik" value="{{ old('nik') }}" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all" placeholder="Masukkan NIK">
            </div>
        ...
        <div class="max-w-md">
            ...
            <input type="text" id="jabatan" name="jabatan" value="{{ old('jabatan') }}" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all" placeholder="Masukkan jabatan (opsional)">
        </div>
