...
@section('title', 'Edit Petugas - Ependri')
...
<div class="bg-white rounded-2xl shadow-xl overflow-hidden">
    ...
    <div class="px-6 py-5 bg-gradient-to-r from-blue-500 to-blue-600 flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-white">Edit Petugas</h1>
            <p class="text-blue-100 text-sm mt-1">Perbarui data petugas</p>
        </div>
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('admin.petugas.update', $petugas->id) }}" class="p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
                <label for="nip" class="block text-sm font-semibold text-gray-700 mb-2">
                    <i class="fas fa-id-badge mr-2 text-blue-500"></i>NIP *
                </label>
                <input type="text" id="nip" name="nip" value="{{ old('nip', $petugas->nip) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="Masukkan NIP">
            </div>
            ...
            <div>
                ...
                <input type="text" id="nama" name="nama" value="{{ old('nama', $petugas->nama) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="Masukkan nama lengkap">
            </div>
            ...
            <div>
                ...
                <input type="email" id="email" name="email" value="{{ old('email', $petugas->email) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="Masukkan email">
            </div>
            ...
            <div>
                ...
                <input type="text" id="jabatan" name="jabatan" value="{{ old('jabatan', $petugas->jabatan) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="Masukkan jabatan">
            </div>
            ...
            <div>
                ...
                <select id="instansi_id" name="instansi_id" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                    ...
                    <option value="{{ $ins->id }}" {{ old('instansi_id', $petugas->instansi_id) == $ins->id ? 'selected' : '' }}>{{ $ins->nama }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" id="telp" name="telp" value="{{ old('telp', $petugas->telp) }}" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="Contoh: 08xxxxxxxxxx">
            </div>
        </div>
